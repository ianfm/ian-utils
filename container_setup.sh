...
git config --global user.email "ian@farmx.co"
...
if [ -f ~/.ssh/id_rsa_farmx_github ]; then
    echo "key exists, skipping keygen"
else
    ssh-keygen -t rsa -b 4096 -C "ian@farmx.co" -f "/home/ubuntu/.ssh/id_rsa_farmx_github" -N ""
    # eval "$(ssh-agent -s)"
    # ssh-add /home/ubuntu/.ssh/id_rsa_farmx_github
fi


# Copy key to github
# cat ~/.ssh/id_rsa_farmx_github.pub
...
gh ssh-key add "/home/ubuntu/.ssh/id_rsa_farmx_github.pub" --title $CONTAINER_ID 
...
echo '"ros.rosSetupScript": "/home/ubuntu/.bashrc",
